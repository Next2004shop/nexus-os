import { Home, BarChart2, Wallet, User, Bell, Search, Building, TrendingUp, Bot } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

import { HelpCenterPage } from '../../pages/HelpCenterPage';

// eslint-disable-next-line no-unused-vars
const NavItem = ({ icon: Icon, label, id, active, onNavigate }) => (
    <button
        ...
        <div className="min-h-screen bg-nexus-black text-nexus-text font-sans selection:bg-nexus-blue/30">
            ...
            <aside className="hidden md:flex flex-col w-64 fixed left-0 top-0 bottom-0 bg-nexus-black/50 backdrop-blur-xl border-r border-white/5 z-50">
                ...
                <nav className="flex-1 px-4 space-y-2 mt-4">
                    <NavItem icon={Home} label="Dashboard" id="home" active={activeTab === 'home'} onNavigate={onNavigate} />
                    <NavItem icon={BarChart2} label="Trade" id="trade" active={activeTab === 'trade'} onNavigate={onNavigate} />
                    <NavItem icon={Wallet} label="Wallet" id="wallet" active={activeTab === 'wallet'} onNavigate={onNavigate} />
                    <NavItem icon={Building} label="Banking" id="banking" active={activeTab === 'banking'} onNavigate={onNavigate} />
                    <NavItem icon={TrendingUp} label="Investments" id="investments" active={activeTab === 'investments'} onNavigate={onNavigate} />
                    <NavItem icon={Bot} label="AI Bot" id="aibot" active={activeTab === 'aibot'} onNavigate={onNavigate} />
                    <NavItem icon={User} label="Profile" id="profile" active={activeTab === 'profile'} onNavigate={onNavigate} />
                </nav>
            ...
            <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-nexus-black/80 backdrop-blur-xl border-t border-white/10 z-50 px-6 py-2 pb-safe flex justify-between items-center">
                <NavItem icon={Home} label="Home" id="home" active={activeTab === 'home'} onNavigate={onNavigate} />
                <NavItem icon={BarChart2} label="Trade" id="trade" active={activeTab === 'trade'} onNavigate={onNavigate} />
